...
var BridgeAddress = ethcmn.HexToAddress("0xcB76d991cFCd621b477d705be7DdF5EA69D39C00")
...
func TestContractCode(t *testing.T) {
	...
	byteCode, err := conn.getByteCode(BridgeAddress)
	if err != nil {
		t.Fatal(err)
	}
	t.Logf("\nBridgeBin Length: %d\n BridgeRunTimeBytecode Length %d\n CodeAt result length %d\n", len(Bridge.BridgeBin), len(Bridge.RuntimeBytecode), len(hex.EncodeToString(byteCode)))
	if len(byteCode) == 0 || Bridge.RuntimeBytecode != "0x"+hex.EncodeToString(byteCode) {
		t.Fatal("Bridge Contract doesn't exist")
	}
